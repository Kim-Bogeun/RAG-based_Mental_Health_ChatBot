...
async def search_similar_and_build_prompt(
    ...
    for idx, it in enumerate(similar_items, 1):
        ...
        for ridx, ex in enumerate(it["reframes"], 1):
            ...
            prompt_parts.append(f"Example Thought   {ridx}: {ex['thought']}")
            prompt_parts.append(f"Example Reframe   {ridx}: {ex['reframe']}")

        prompt_parts.append("")  # 빈 줄

    # ─── Part 3: 출력 포맷 지시 ───
    prompt_parts.append(
        "3. Now generate new reframed thoughts for the user's input (based on their Situation & Thought). "
    ...
    for idx in range(1, len(similar_items) + 1):
        ...
        prompt_parts.append(f"Reframed Thoughts for Distortion {idx}:\n")
